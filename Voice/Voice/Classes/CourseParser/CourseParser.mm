...
//        NSLog([NSString stringWithFormat:@"%@/model.dat", dataPath]);
        [isaybios ISAYB_SetModel:[[NSString stringWithFormat:@"%@/model.dat", dataPath] cStringUsingEncoding:NSUTF8StringEncoding]];
...
- (void)loadCourses:(NSString*)filename
{
	...
	tbxml = [TBXML tbxmlWithXMLData:filedata];
...
//        }
        ...
        tbxml = [TBXML tbxmlWithXMLData:[NSData dataWithBytes:filedata length:nLen]];
